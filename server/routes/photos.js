...
router.get("/", async (req, res) => {
  try {
    const [rows] = await pool.query("SELECT id, url, title, title_en, sort_order, created_at, updated_at FROM photos ORDER BY sort_order ASC");
    res.json(rows);
  } catch (err) {
...
router.put("/:id", idRule, partialMediaRules, handleValidation, async (req, res) => {
  try {
    ...
    const { url, title, titleEn, sortOrder } = req.body;
    const [result] = await pool.query(
      "UPDATE photos SET url = COALESCE(?, url), title = COALESCE(?, title), title_en = COALESCE(?, title_en), sort_order = COALESCE(?, sort_order) WHERE id = ?",
    ...
    const [updated] = await pool.query("SELECT id, url, title, title_en, sort_order, created_at, updated_at FROM photos WHERE id = ?", [id]);
    res.json(updated[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.delete("/:id", idRule, handleValidation, async (req, res) => {
  try {
    const { id } = req.params;
    const [rows] = await pool.query("SELECT url FROM photos WHERE id = ?", [id]);
